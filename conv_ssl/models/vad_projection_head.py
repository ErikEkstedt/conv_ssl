...
class VadProjection(nn.Module):
    ...
    def init_first_speaker_mapping(
        self, both_is_valid_target=True
    ...
    def init_next_speaker_idx(self):
        """
        Change speaker idx
    ...
    def topk_hold_shift(self, probs, w_ab, w_ba, w_aa, w_bb):
        ...
        # HOld
        hold_topk = torch.zeros((*probs.shape[:-1], self.k), dtype=torch.long)
        ...
        score["shift"] = {
            "topk": torch.cat((topk_ab_correct, topk_ba_correct)).sum(dim=0),
            "n": n_shift,
        }
        ...
        score["hold"] = {
            "topk": torch.cat((topk_aa_correct, topk_bb_correct)).sum(dim=0),
            "n": n_hold,
        }
        #
        ...
        data = {}
        data["hold_topk"] = hold_topk
        data["hold_topk_probs"] = hold_topk_probs
        data["hold_topk_correct"] = hold_topk_correct
        data["shift_topk"] = shift_topk
        data["shift_topk_probs"] = shift_topk_probs
        data["shift_topk_correct"] = shift_topk_correct
        return data, score

    def probs_hold_shift(self, probs, w_ab, w_ba, w_aa, w_bb):
        shift_probs = torch.zeros(probs.shape[:-1], dtype=torch.float)
        hold_probs = torch.zeros(probs.shape[:-1], dtype=torch.float)
    ...
    def get_hold_shift(self, probs, vad, metrics=["probs", "topk"]):
        ...
        ba = torch.logical_and(shift_oh, b_last)
        ab = torch.logical_and(shift_oh, a_last)
        aa = torch.logical_and(hold_oh, a_last)
        bb = torch.logical_and(hold_oh, b_last)
        w_ab = torch.where(ab)
        w_ba = torch.where(ba)
        w_aa = torch.where(aa)
        w_bb = torch.where(bb)
        ...
        if "probs" in metrics:
            ...
            score["probs"] = score_
            # ret["data"]["probs"] = data
            # ret["score"]["probs"] = score
        return data, score

    def get_change_probs(self, probs, a_current, b_current):
        # Speaker Change Probs
        change_prob = torch.zeros(tuple(probs.shape[:-1]), dtype=torch.float)
    ...
    def get_eot_probs(self, probs, a_current, b_current, vad):
        eot_probs = torch.zeros(probs.shape[:-1], dtype=torch.float)
    ...
    def forward(self, logits, vad, metrics=["topk", "probs"]):
        ...
        # ret["change_probs"] = self.get_change_probs(probs, a_current, b_current)
        # ret["eot_probs"] = self.get_eot_probs(probs, a_current, b_current)
        return data, score
...
def speaker_change_vector_debug():
    ...
    next_speaker_a_oh = vad_projection_head(next_a_idx)

    first_speaker_a = vad_projection_head.first_speaker_idx[0]
    first_speaker_a_oh = vad_projection_head(first_speaker_a)
    ####################################################################
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            plot_vad_oh(first_speaker_a_oh[j].permute(1, 0), ax=ax[row, col])
            j += 1
    ...
    for row in range(10):
        for col in range(5):
            plot_vad_oh(next_speaker_a_oh[j].permute(1, 0), ax=ax[row, col])
            j += 1
    ax[0, 0].set_xlim([0.5, 3.5])
    ax[0, 0].set_xticks([])
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    fig, ax = plot_vad_projection(
        vad=vad[b],
        vad_history=vad_history[b],
        shift_probs=shift_probs[b],
        hold_probs=hold_probs[b],
        change_probs=change_probs[b],
        eot_probs=eot_probs[b],
        probs=probs[b],
        hold_oh=hold_oh[b],
        shift_oh=shift_oh[b],
        eot_pos_oh=eot_pos_oh[b],
        eot_neg_oh=eot_neg_oh[b],
